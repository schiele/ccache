         ...
         struct hash* depend_mode_hash)
{
  ...
  if (ctx.guessed_compiler == GuessedCompiler::msvc) {
    const char* tmp_stderr2 = format("%s.2", tmp_stderr.c_str());
    if (x_rename(tmp_stderr.c_str(), tmp_stderr2)) {
    ...
    if (!result_stream.is_open()) {
      cc_log("Failed opening %s: %s", tmp_stderr.c_str(), strerror(errno));
      failed(STATS_ERROR);
    }

    std::ostreambuf_iterator<char> to(result_stream);
    for (const char* file : {tmp_stdout.c_str(), tmp_stderr2}) {
    ...
    if (!result_stream.good()) {
      cc_log("Failed at writing data into %s: %s",
             tmp_stderr.c_str(),
             strerror(errno));
      failed(STATS_ERROR);
    }
    ...
    free(const_cast<char*>(tmp_stderr2));
  }
